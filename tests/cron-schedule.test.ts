...
import {
  ...
  parseCronExpression,
} from "../workers/main/src/cron-schedule";
...
describe("cron-schedule", () => {
  ...
  it("computes minimum interval for billing frequency gates", () => {
    const from = Date.UTC(2026, 1, 24, 10, 0, 0);
    expect(getCronMinimumIntervalMs("0 * * * *", from)).toBe(60 * 60 * 1000);
    expect(getCronMinimumIntervalMs("0 0 * * *", from)).toBe(
      24 * 60 * 60 * 1000,
    );
  });
});

# Chat Explorer Errors Dashboard Combined Handoff

Combined from:

- `/Users/illiana/conductor/workspaces/chiridion-app/chat-explorer-plan/docs/chat-explorer-errors-dashboard-implementation-feedback.md`
- `/Users/illiana/conductor/workspaces/chiridion-app/chat-explorer-plan/docs/chat-explorer-errors-dashboard-fix-architecture.md`
...
## Follow-Up Bug Fix Architecture

This section covers the three follow-up bugs identified after the first implementation pass. The goal is a small, durable fix that keeps the admin index fast while making the metadata trustworthy.
...
2. Preserve existing D1 error/model metadata on partial or older-style `thread_upsert` payloads.
3. Add direct-send error recording through a central helper and replace the two user-message send failure branches.
4. Decide whether fallback-discovered persisted assistant errors should insert bounded `thread_error_recorded` rows, or explicitly document that historical fallback errors are Chat Explorer-only.
5. Rewrite `getChatErrorThreads()` around a grouped-by-thread CTE.
6. Add focused regression tests for each bug before broad cleanup.

## Verification From Review Pass

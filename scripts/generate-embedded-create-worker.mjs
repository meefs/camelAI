...
const OUT_PATH = resolve(ROOT_DIR, 'workers/main/src/embedded-create-worker.ts');
...
function buildTarGzipBase64(entries) {
  ...
  for (const entry of entries) {
    const archivePath = `create-worker/${entry.path}`;
...
function toOutput(version, archiveBase64) {
  return `/**\n` +
    ` * Embedded tar.gz payload for the create-worker CLI tool.\n` +
    ` * Extracted to /usr/local/lib/create-worker/ on sprite bootstrap.\n` +
    ` * Generated from sandbox/create-worker.\n` +
    ` */\n\n` +
    `export const EMBEDDED_CREATE_WORKER_VERSION = ${JSON.stringify(version)};\n\n` +
    `export const EMBEDDED_CREATE_WORKER_ARCHIVE_BASE64 = ${JSON.stringify(archiveBase64)};\n`;
}

async function main() {
  const entries = await collectFiles(CREATE_WORKER_DIR);
  ...
  if (currentOutput === nextOutput) {
    console.log('[gen:embedded-create-worker] up to date');
    return;
  }

  await writeFile(OUT_PATH, nextOutput, 'utf8');
  console.log('[gen:embedded-create-worker] updated workers/main/src/embedded-create-worker.ts');
}

main().catch((error) => {
  console.error('[gen:embedded-create-worker] failed:', error);
  process.exit(1);
});

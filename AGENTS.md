...
13. `ChatThreadDO` scans browser and external turns for both canonical `(user uploaded file to /mnt/user-uploads/...)` references and raw `/mnt/user-uploads/...` path mentions, then prepends a hidden file-safety `<camelai system message>` before author attribution when any file looks unsafe.

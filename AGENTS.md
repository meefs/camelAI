...
- Browser chat transport is HTTP + SSE (no WebSocket since 2026-08): the client receives frames over `GET /agents/chat-thread/:threadId/sse` (one stream per thread view, `SseAgentClient` in `src/lib/sse-agent-client.ts`) and sends RPCs/resume frames via `POST /agents/chat-thread/:threadId/call`. The legacy `/agents/chat-thread/:threadId` WebSocket upgrade path remains server-side for stale bundles only; remove it once prod telemetry shows zero WS attaches. Client SSE lifecycle telemetry is `chat_sse_*` / `chat_post_*` (`src/lib/chat-sse-telemetry.ts`); server auth events keep the `chat_ws_auth_completed`/`chat_ws_upgrade_rejected`/`chat_ws_upgrade_degraded` names. Workspace status streams over `GET /api/workspaces/:id/status/stream` (snapshot always first).
...
- The build container sleeps when idle and takes 30-120s to wake, far longer than the deploy retry ladder. `deploy_project`/`add_dependency` therefore run `ensureBuildSandboxReady` (`project-build-readiness.ts`) before their first sandbox call — one cheap `exists` probe when warm, a budgeted re-probe loop when cold — and the existing 5-attempt ladder stays as the guard for post-readiness blips (it re-arms the gate between attempts, under one shared budget).

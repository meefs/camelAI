...
Sandbox notebook execution preloads pandas display defaults through IPython startup (`display.max_rows=200`, `display.max_columns=50`, `display.max_colwidth=200`, `display.width=None`) so notebook HTML outputs include richer table content without per-notebook `pd.set_option` boilerplate.

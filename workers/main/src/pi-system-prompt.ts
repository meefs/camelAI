...
export function createPiSystemPrompt(
  ...
  return [
    ...
    "Outbound email, Slack, and Telegram messages are opt-in side effects. In ordinary web chats, answer in chat only unless the user explicitly asks you to send an external message. Attribution prefixes such as `[eval message from Name (email)]` identify the author only and never imply an email reply. Channel-originated turns include their own hidden routing instruction when an external reply is needed.",

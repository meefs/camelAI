...
export function createPiSystemPrompt(
  ...
  return [
    ...
    "Projects are source/build/deploy work areas backed by durable project storage and Cloudflare Artifacts history. Project names are unique within the workspace and are the handle to use in tools. Use `list_projects`, `create_project`, `scaffold_project`, and `set_project_description` to discover, create, scaffold, or describe projects. New projects are DO-backed (`backend: \"do-r2\"`) and `create_project` seeds a deployable React Router SSR app with Tailwind/shadcn-style UI primitives by default; pass `template: \"worker\"` only when the user specifically needs a bare Worker scaffold. Edit files with `location: \"project\"`, use `add_dependency` for package changes, `build_project` for validation builds, `deploy_project` for builds plus deploys, `list_commits` for source history, `revert_project` for source restore, and `rollback_deploy` for live-site rollback. Do not call `build_project` before `deploy_project` unless you only want validation, because `deploy_project` always builds first. Use `scaffold_project` on an existing empty DO-backed project if package.json/build files are missing. Build scripts must define `scripts.build` and declare every CLI they use in dependencies/devDependencies. New projects require a concise description, and `list_projects` includes descriptions and backend values.",

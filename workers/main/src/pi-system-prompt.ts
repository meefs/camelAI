...
export function createPiSystemPrompt(
  ...
  return [
    ...
    "Projects are source/build/deploy work areas backed by durable project storage and Cloudflare Artifacts history. Project names are unique within the workspace and are the handle to use in tools. Use top-level `list_projects`, `create_project`, and `set_project_description` to discover, create, or describe projects. REQUIRED: before the first `create_project` call in a task, call `read` with `location: \"workspace\"` and `path: \"/opt/chiridion-host-pi/skills/developing-software/SKILL.md\"`, then read the entire skill. Skip that read only if it already succeeded during the current task. Do not create a scaffold first and read the skill afterward. New projects are DO-backed (`backend: \"do-r2\"`). Choose the closest `create_project` template: `crud` (the default) for stateful product and internal-tool work, with a working React Router + Durable Object SQLite CRUD flow; `ai-chat` for an assistant using the virtual AI binding; `integration-dashboard` for apps centered on workspace connections; `data-dashboard` for interactive metrics, charts, and tables; or `data-analysis` for a notebook-first report. The `data-analysis` template seeds a Report-mode `analysis.ipynb`, runs with `run_notebook`, and publishes through `deploy_project` as a static read-only report app with no build step. Edit files with `location: \"project\"`. Other project operations are available inside `js_exec` on the `tools` object: use `await tools.add_dependency(...)` for package changes, `await tools.add_shadcn_component(...)` for the full bundled shadcn/ui catalog — components and complete blocks (login pages, sidebar layouts, dashboards) with dependencies resolved and npm packages added automatically; always prefer it over hand-writing standard UI components, `await tools.build_project(...)` for validation builds, `await tools.deploy_project(...)` for builds plus deploys, `await tools.list_commits(...)` for source history, `await tools.revert_project(...)` for source restore, and `await tools.rollback_deploy(...)` for live-site rollback. Do not use create-worker, wrangler, or package-manager shell commands for project scaffolding, builds, deploys, or bundled shadcn components. Do not call `build_project` before `deploy_project` unless you only want validation, because `deploy_project` always builds first. If a project is missing package.json/build files, create a new project (which seeds a full scaffold) instead of trying to scaffold in place. Build scripts must define `scripts.build` and declare every CLI they use in dependencies/devDependencies. New projects require a concise description, and `list_projects` includes descriptions and backend values.",

...
// Keep this runtime facade aligned with
// sandbox/create-worker/templates/starter/app/lib/connections.ts. The deployed
// CONNECTIONS binding is a WorkerEntrypoint RPC stub: dynamic property access
// such as env.CONNECTIONS[alias] is interpreted as an RPC method named after
// the alias. Apps must route method-style calls through invoke(), which this
// helper does while preserving normal fetch(input, init) ergonomics.
const PROJECT_CONNECTIONS_HELPER_SOURCE = `type ConnectionMethodCatalogEntry = {
  alias: string;
  connection: {
    id: string;
    type: string;
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        `## Workflow`,
        ``,
        `- Execute with \`run_notebook({ project: ${JSON.stringify(projectName)}, path: "analysis.ipynb" })\` — one call runs the notebook, validates the outputs, persists results back to the project, and opens a clean successful run in preview automatically. If \`ok\` is false, the current preview is unchanged; fix the failing cells (see \`validation.issues\`/\`stderr\`) and re-run.`,
        ...
        `- No manual \`set_preview\` call is needed after a clean run; use it only to reopen or switch to an existing notebook explicitly.`,
        `- Only when the user explicitly asks to publish or share the report outside chat, \`deploy_project({ project: ${JSON.stringify(projectName)}, publish_intent: "user_requested" })\` publishes the executed notebook as a static read-only report app and opens it in preview automatically (the platform notebook renderer plus the .ipynb — no build step). Always run \`run_notebook\` first so the published outputs are fresh. No manual \`set_preview\` or \`list_apps\` call is needed after deploy, though \`set_preview\` remains available for an explicit preview switch.`,
        ``,
        `## Report conventions`,
        ``,
...
function defaultReactRouterScaffoldFiles(projectName: string, scriptName: string): ProjectScaffoldFile[] {
  ...
  return [
    ...
    {
      path: "/app/lib/connections.ts",
      content: PROJECT_CONNECTIONS_HELPER_SOURCE,
    },

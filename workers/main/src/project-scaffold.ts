  ...
  return [
    ...
    },
    {
      ...
      content: file([
        ...
        ``,
        "Durable Object SQLite is NOT the D1 API. `this.ctx.storage.sql` has no `.prepare()`, `.bind()`, `.all()`, `.first()`, or `.run()` — D1-style calls build cleanly (the build does not typecheck) and crash at runtime after deploy. Pass parameters directly to `exec` and read the cursor:",
        ``,
        "```ts",
        ...
        "```",
        ``,
        "Run `bun run typecheck` to catch API misuse before deploying.",
      ]),
    },
  ];
}

...
const CODE_MODE_TOOL_REGISTRY: CodeModeToolRegistration[] = [
  ...
  codeModeTool(
    "vm_exec",
    "Run a command in a project VM. Prefer the js_exec vm.exec(command, options) facade. Commands run from /workspace by default; pass cwd only for subdirectories in that checkout. Arguments: { command, project, location?: 'vm', cwd?, timeoutMs?, timeoutSeconds?, env? }.",
    Type.Object({
      command: Type.String(),
      ...vmTargetParameters(),
      cwd: Type.Optional(Type.String()),

    ...
    const definitions: AgentTool[] = [
      ...
      },
      {
        ...
        description:
          ...
          "AI globals: `env.AI` and `context.cloudflare.env.AI` expose the virtual AI binding (`run()` only). Call `await env.AI.run(\"auto\", { messages: [{ role: \"user\", content: \"hello\" }] })` for text or `auto_search` for grounded search. For images, call `await generateImage(env.AI, \"prompt\")` or `await generateImage(env.AI, { prompt, referenceImageUrl })` — `generateImage` is a js_exec global; in deployed user-worker TypeScript import it from `workers/camelai-ai.ts`. Returns `{ text, imageDataUrl, images }`. " +

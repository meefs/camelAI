    ...
    const definitions: AgentTool[] = [
      ...
      },
      {
        ...
        description:
          ...
          "AI globals: `env.AI` and `context.cloudflare.env.AI` expose the same virtual AI binding as deployed user workers. Call `await env.AI.run(\"auto\", { messages: [{ role: \"user\", content: \"hello\" }] })` for text or `auto_search` for grounded search. For images, call `await env.AI.generateImage(\"prompt\")` or `await env.AI.generateImage({ prompt, referenceImageUrl })`; it returns `{ text, imageDataUrl, images }`. " +

...
export class ChatThreadDO extends DurableObject<ChatEnv> {
  ...
  private createPiSystemPrompt(context: ChatContextState): string {
    ...
    return [
      ...
      "For hosted AI in `js_exec`, use `env.AI` or `context.cloudflare.env.AI` the same way as deployed user workers, for example `await env.AI.run(\"auto\", { messages: [{ role: \"user\", content: \"hello\" }] })`. Model routes include `auto` and `auto_search`. For image generation in `js_exec`, use `await env.AI.generateImage(\"prompt\")`. In deployed user-worker TypeScript, import `generateImage` from `workers/camelai-ai.ts` and call `generateImage(env.AI, \"prompt\")` because Wrangler `Ai` types only declare `run()`. Do not use `workers-ai-provider` / `generateText()` with `auto_image` because images are dropped.",
    ...
    const definitions: AgentTool[] = [
      ...
      },
      {
        ...
        description:
          ...
          "AI globals: `env.AI` and `context.cloudflare.env.AI` expose the same virtual AI binding as deployed user workers. Call `await env.AI.run(\"auto\", { messages: [{ role: \"user\", content: \"hello\" }] })` for text or `auto_search` for grounded search. For images in `js_exec`, call `await env.AI.generateImage(\"prompt\")` or `await env.AI.generateImage({ prompt, referenceImageUrl })` (returns `{ text, imageDataUrl, images }`). In deployed user-worker TypeScript, use `import { generateImage } from \"../workers/camelai-ai\"` then `await generateImage(env.AI, prompt)` so code typechecks with standard Wrangler `Ai` types. " +

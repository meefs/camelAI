...
import {
  consumeSalesPrompt,
  getPromptKeyFromUrl,
} from "../../../../src/lib/sales-prompt.server.js";
import { isUserBanned } from "../ban-list.js";
...
export async function handleOAuthStart({
  ...
  const clientId = env[config.clientIdEnvVar as keyof Env] as
    | string
    | undefined;
  if (!clientId)
    return text(`${config.displayName} OAuth is not configured`, 500);
...
export async function handleOAuthCallback({
  ...
  if (!code || !state)
    return redirectTo(`${url.origin}/login?error=oauth_invalid`);

  // Verify OAuth state from signed cookie
  const stateData = await getOAuthStateFromRequest(
    req,
    env.TOKEN_SIGNING_SECRET,
  );
  if (
  ...
  const clientId = env[config.clientIdEnvVar as keyof Env] as
    | string
    | undefined;
  const clientSecret = env[config.clientSecretEnvVar as keyof Env] as
    | string
    | undefined;
  if (!clientId || !clientSecret)
    return redirectTo(`${url.origin}/login?error=oauth_config`);
  ...
  try {
    ...
    stage = 'user_lookup';
    const existingBan = await isUserBanned(env.APP_KV, {
      email: userInfo.email,
    });
    if (existingBan) {
      const headers = new Headers({
        Location: `${url.origin}/banned`,
      });
    ...
    stage = "org_workspace";
    const { orgId, workspaceId } = await ensureDefaultOrgWorkspace(
      env,
      userId,
      displayName,
    );
...
function getPromptKeyFromRedirectUrl(

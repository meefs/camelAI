...
// the "domains" category, which the lean tool surface (PI_LEAN_TOOLS=1) drops from the
...
describe("custom domain agent eval", () => {
  maybeIt(
    ...
    async () => {
      ...
      // The core regression check: in lean mode get_custom_domain is not a top-level
      // tool, so the agent must have discovered it via tools.search and invoked it.
      expect(
        calledGetCustomDomain,
        "expected the agent to discover and call get_custom_domain",
      ).toBe(true);

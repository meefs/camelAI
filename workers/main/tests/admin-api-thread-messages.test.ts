...
describe('admin API thread messages route', () => {
  ...
  it('serves parsed thread messages for banned orgs with bearer auth', async () => {
    const email = testEmail();
    const { userId } = await createUser(testEnv, email, 'password123', 'Banned Admin API User');
    const { org, defaultWorkspaceId } = await createOrg(testEnv, 'Banned Admin API Org', userId);
    const orgStub = testEnv.ORG.get(testEnv.ORG.idFromName(org.id));
    const thread = await orgStub.createThread(defaultWorkspaceId, 'Banned org thread messages', userId);

    await putOrgBan(testEnv.APP_KV, {
      scope: 'org',
      target_id: org.id,
      email: null,
      org_slug: org.slug,
        ...
        messages: [
          {
            ...
            thread_id: thread.id,
            role: 'user',
            content: [{ type: 'text', text: 'please audit this' }],
            created_at: 1730000000000,
          },
        ],
      });

...
describe('pi_core pruning', () => {
  function pruningStore(rowCount: number) {
    const statements: Array<{ sql: string; args: unknown[] }> = [];
    let remaining = rowCount;
    const exec = (sql: string, ...args: unknown[]) => {
      statements.push({ sql, args });
      if (sql.includes('COUNT(*) AS count')) {
        const bound = Number(args[0] ?? 0);
        return { toArray: () => [{ count: Math.min(remaining, Math.max(0, bound)) }] } as never;
      }
      if (sql.startsWith('DELETE FROM pi_core_messages WHERE idx <')) {
        remaining = Math.max(0, remaining - Number(args[0] ?? 0));
      }
  ...
  it('deletes only rows below the bound', () => {
    const { store, statements } = pruningStore(100);

    const pruned = store.prunePiCoreMessagesBelow(40);

    expect(pruned).toBe(40);
    const del = statements.find((s) => s.sql.startsWith('DELETE FROM pi_core_messages WHERE idx <'));
    expect(del).toBeDefined();
    expect(del?.args).toEqual([40]);
  });

  it('is a no-op for a zero or negative bound', () => {
    // The safety property that keeps environments without a transcript-lake
    // archive intact: the lake mark never advances there, so the clamped bound
    // is 0 and nothing is ever deleted.
    for (const bound of [0, -1, Number.NaN]) {
      const { store, statements } = pruningStore(100);
      expect(store.prunePiCoreMessagesBelow(bound)).toBe(0);
      expect(statements.some((s) => s.sql.startsWith('DELETE'))).toBe(false);
    }
  });

  it('issues no DELETE when nothing is below the bound', () => {
    const { store, statements } = pruningStore(0);

    expect(store.prunePiCoreMessagesBelow(50)).toBe(0);
    expect(statements.some((s) => s.sql.startsWith('DELETE'))).toBe(false);
  });
});

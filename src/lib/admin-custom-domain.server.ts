...
export async function refreshOrgCustomDomainHostnamesForAdmin(
  ...
  for (const script of scripts) {
    const hostname = script.custom_domain_hostname;
    if (!hostname) {
    ...
    if (!options.includeActive && isAppCustomDomainReady(script)) {
      result.skipped_active += 1;
      result.apps.push({
        script_name: script.script_name,
        hostname,
    ...
    result.attempted += 1;
    let record = null;
    let appRecordError: string | null = null;
    try {
      record = await createOrRefreshCustomHostname(zoneId, apiToken, hostname);
      if (!record) {
        appRecordError = 'Failed to create or refresh Cloudflare custom hostname';
      }
    } catch (error) {
      appRecordError = error instanceof Error ? error.message : String(error);
    }

    if (!record) {
      const error = appRecordError ?? 'Failed to create or refresh Cloudflare custom hostname';
      try {

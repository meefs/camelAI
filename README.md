...
<p align="center">
  <a href="https://camelai.dev">Website</a> ·
...
camelAI gives teams persistent AI coding workspaces backed by Cloudflare. Users
can chat with Claude- or Codex-backed agents, connect external services and data,
build applications in isolated sandboxes, and publish them to managed app hosts.

The platform includes:

- persistent chat threads and project files
- AI coding agents with workspace-aware tools
- integrations for APIs, databases, email, Slack, and other services
...
React Router SSR + browser WebSocket
       ...
       Cloudflare main Worker
        ...
        |                    |
        v                    v
WorkspaceFilesystemDO   Cloudflare sandbox
      + R2              containers
 project source       build / analysis / SQL
        |
        v
Dispatcher Worker -> published *.camelai.app applications
```

`ChatThreadDO` owns agent turns and persistent chat state. Project source files
live in `WorkspaceFilesystemDO` and R2. Builds, deployments, notebook analysis,
and database queries run in dedicated Cloudflare sandbox containers. The
dispatcher routes requests to published user applications.

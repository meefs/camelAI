/**
 * Typed image generation for camelAI virtual AI bindings.
 *
 * Wrangler's generated `Env` types `AI` as Cloudflare's `Ai`, which only declares
 * `run()`. On camelAI deploys the binding also exposes `generateImage()` at runtime,
 * but calling it directly fails TypeScript unless you augment types. Use
 * `generateImage(ai, prompt)` here — it only needs `run()` and typechecks with
 * standard `Env`.
 */
...
export type CamelAiBinding = AiRunBinding & {
...
/** Attach a typed `generateImage` method when you prefer method-style calls. */
export function asCamelAi(ai: AiRunBinding): CamelAiBinding {
  return Object.assign(ai, {
    generateImage: (input: string | GenerateImageOptions) =>
      generateImage(ai, input),
  });
}
